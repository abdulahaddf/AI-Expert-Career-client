    ...
    <div className="mt-12">
      <div className="flex justify-between items-center">
        <div className="flex items-center gap-3 py-5">
          ...
          <EventTitle
            title={language == "bn" ? "ইভেন্ট স্পন্সরস" : "Event Sponsor"}
          />
        </div>
        <button className="bg-[#D70404] my-btn px-8 py-3 font-semibold text-white rounded-lg">
          {language == "bn" ? "ফ্রি রেজিস্টার করন" : "Register"}
        </button>
      </div>
      <br />
      <div className="grid grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
        {sponsor.map((item) => (
          <img className="" key={item.id} src={item.img} alt="" />
        ))}
      </div>
    </div>
  );
