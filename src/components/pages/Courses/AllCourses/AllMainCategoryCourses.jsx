...
const AllMainCategoryCourses = () => {
    ...
    const filteredCourses = courses?.filter((course) => course.mainCategory == mainCategory);
    ...
    return (
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5 ">
            {
                filteredCourses?.map(course => <CourseCard key={course._id} course={course}></CourseCard>)
            }
        </div>
    </div>
    );
